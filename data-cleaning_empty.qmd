...
```
- all of these are characters not numeric
...
# snow cover column we have whole numbers and then decimals 
```

Let's focus on the non-numeric values as a starting point:
- lots of values focus on thsoe that are not numeric 
...
```{r}
# a . i snow there for every cover that we have 

snowsurvey_csv %>%
  filter(Snow_cover == ".") %>%
  View() 

# returning these to NA might be the best idea 
# gsub or case_whern 
...
```{r}
snow_survery_fixed <- snowsurvey_csv %>%
  #filter(Snow_cover == ".") %>% # filter is good to see whats actually occuring in the datast before applying it 
...
```{r}
# Na
snowsurvey_fixed <-snow_survery_fixed %>% 
  #filter(Snow_cover = "-") %>%
    mutate(Snow_cover = ifelse(Snow_cover==".", NA, Snow_cover)) 
...
snowsurvey_fixed <-snow_survery_fixed %>% 
  #filter(Snow_cover = "<1") %>%
    mutate(Snow_cover = ifelse(Snow_cover=="<1", "0", Snow_cover)) #working with character so coerce with 0  
```
...
# now we have a number for snow_cover 
```
...
- data cleaning is opinionated no right or wrong answer 
- 100 and 0 land cover is strange what does this mean? 
...
```{r}
species_csv <- read_csv(file.path(datadir_raw,
                                      species_file))
glimpse(species_csv)

# every observation is related a report 
# every species has a column / observation 
# we havbe lots of info due to contruction of table 
```
...
- make dataset longer (pivot_longer)

### Data cleaning

```{r}
# 300 something columns of species so exlude the ones you would not want 
species_long <- species_csv %>%
 ...
 ) # telling R not to collapse into one coolumn, should stay in own column 
```
